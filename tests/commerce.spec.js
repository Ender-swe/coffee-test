...
test("guest can shop, change shipping, and reach payment without an account", async ({
  page,
}, info) => {
  const errors = [];
  ...
  await expect(
    page.getByText("Checkout as a guest. An account is always optional."),
  ).toBeVisible();
  ...
  await page.route("https://checkout.stripe.com/**", (route) =>
    route.fulfill({
      ...
      body: "<h1>Stripe payment step</h1>",
    }),
  );
  await page
...
test("bag survives optional signup, form validates and password visibility works", async ({
  page,
}, info) => {
  await page.goto("/shop.html");
  await page.getByRole("button", { name: "Add to your bag" }).first().click();
  ...
  await expect(
    page.getByRole("link", { name: "Continue as a guest" }),
  ).toBeVisible();
  await page.screenshot({
    path: `test-results/signup-${info.project.name}.png`,
    fullPage: true,
  });
  await page.route("**/api/auth/signup", (route) =>
    route.fulfill({
      status: 201,
      json: {
        user: { id: "test", name: "Alex Regular", email: "alex@example.com" },
      },
    }),
  );
...
test("login shows recoverable errors and offers guest checkout", async ({
  page,
}, info) => {
  ...
  await expect(
    page.getByRole("button", { name: "Let’s get you settled" }),
  ).toBeEnabled();
  await page.screenshot({
  ...
  await expect(
    page.getByText("Your bag is waiting for a new favorite."),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Continue to secure payment" }),
  ).toBeDisabled();
});
test("payment failure preserves bag and pages fit the viewport", async ({
  page,
}) => {
  await page.goto("/shop.html");
  await page.getByRole("button", { name: "Add to your bag" }).first().click();
...
test("confirmation requires server-confirmed payment and clears purchased items once", async ({
  page,
}) => {
    ...
    route.fulfill({
      json: {
        order: {
          ...
          status: "paid",
          total: 2295,
          items: [{ name: "The daily ritual", quantity: 1, price: 1800 }],
        },
      },
    }),
  );
