...
export async function syncPayment(db, session) {
  ...
  if (session.payment_status === "paid") {
    await db.query(
      `UPDATE orders SET status='paid', total=$3, shipping_address=$4, paid_at=COALESCE(paid_at,now())
    ...
    await db.query(
      "UPDATE orders SET status='expired' WHERE id=$1 AND stripe_session_id=$2 AND status='pending'",
      [id, session.id],
    );
  }
}
export function registerCheckout(
  app,
  { db, stripe, webUrl, automaticTax, limiter },
) {
  app.post("/api/checkout", limiter, async (req, res) => {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "Please check your contact details and try again." });
    let order;
    try {
      order = quote(req.body.items, req.body.shipping);
    } catch (error) {
    ...
    if (!stripe)
      return res
        ...
        .json({
          error:
            "Payments are not available just yet. Your bag is saved; please try again soon.",
        });
    const user = await currentUser(db, req);
    const requestHash = hash(
      JSON.stringify({
        items: order.lines.map((p) => ({
          id: p.id,
          quantity: p.quantity,
          price: p.price,
        })),
    ...
    try {
      ...
      await client.query(
        `INSERT INTO orders(id,access_hash,request_hash,user_id,email,name,items,subtotal,shipping,total,shipping_method)
      ...
      if (
        saved.access_hash !== hash(accessToken) ||
        ...
        return res
          .status(409)
          .json({
            error: "Your bag has changed. Refresh checkout and try again.",
          });
      }
      ...
      if (saved.status !== "pending") {
        ...
        return res
          ...
          .json({
            error:
              "This checkout has already finished. Start a new checkout from your bag.",
          });
      }
      if (saved.stripe_session_id) {
        ...
        if (existing.status === "expired") {
          ...
          return res
            ...
            .json({
              error:
                "That payment session expired. Your bag is saved; click continue again for a fresh checkout.",
            });
        }
      }
      ...
      if (!saved.stripe_session_id) {
        const session = await stripe.checkout.sessions.create(
          {
            ...
            line_items: order.lines.map((p) => ({
              quantity: p.quantity,
              price_data: {
                currency: "usd",
                unit_amount: p.price,
                tax_behavior: "exclusive",
                product_data: { name: p.name, description: p.subtitle },
              },
            })),
            ...
            shipping_options: [
              {
                shipping_rate_data: {
                  ...
                  display_name: order.option.name,
                  tax_behavior: "exclusive",
                },
              },
            ],
        ...
        url = session.url;
        await client.query(
          "UPDATE orders SET stripe_session_id=$2,stripe_url=$3 WHERE id=$1",
          [key, session.id, url],
        );
      }
      await client.query("COMMIT");
  ...
  app.get("/api/orders/:id", limiter, async (req, res) => {
    ...
    if (!uuid.test(req.params.id) || !/^[a-f0-9]{64}$/.test(token))
      return res.status(404).json({ error: "Order not found on this device." });
    const {
      rows: [order],
    } = await db.query("SELECT * FROM orders WHERE id=$1 AND access_hash=$2", [
      req.params.id,
      hash(token),
    ]);
    if (!order)
    ...
    const {
      ...
      "SELECT id,status,total,items,shipping_method FROM orders WHERE id=$1",
      [order.id],
    );
