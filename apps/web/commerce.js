...
const paths = {
  ...
  truck: '<path d="M2 5h12v13H2V5Zm12 5h4l4 5v3h-8"/><circle cx="6" cy="19" r="2"/><circle cx="18" cy="19" r="2"/>',
};
const icon = name => `<svg class="c-icon" viewBox="0 0 24 26" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${paths[name] || paths.flower}</svg>`;
...
async function api(path, options = {}) {
  ...
  catch { throw new Error('We could not connect. Check your connection and try again.'); }
  let data;
  try { data = await response.json(); } catch { throw new Error('Our shop is temporarily unavailable. Please try again soon.'); }
...
function updateBag() { document.querySelector('#bag-count').textContent = getCart().reduce((sum, p) => sum + p.quantity, 0); }
...
function renderBag() {
  ...
  if (!cart.length) {
    ...
    document.querySelectorAll('[data-shipping-price]').forEach(el => el.textContent = money(shippingOptions.find(s => s.id === el.dataset.shippingPrice).amount));
    return;
  }
  const order = quote(cart, shipping);
  items.innerHTML = order.lines.map(p => `<article class="bag-item">${artwork(p, true)}<div class="bag-item-info"><h3>${p.name}</h3><p>${p.subtitle}</p><div class="quantity"><button type="button" data-quantity="${p.id}" data-delta="-1" aria-label="Decrease ${p.name} quantity">−</button><span aria-label="Quantity">${p.quantity}</span><button type="button" data-quantity="${p.id}" data-delta="1" aria-label="Increase ${p.name} quantity" ${p.quantity === 10 ? 'disabled' : ''}>+</button></div><button class="remove-item" type="button" data-remove="${p.id}">Remove</button></div><b>${money(p.price * p.quantity)}</b></article>`).join('');
  const remaining = Math.max(4000 - order.subtotal, 0);
  document.querySelector('#bag-totals').innerHTML = `<div class="free-shipping">${icon(remaining ? 'truck' : 'check')}<span>${remaining ? `You’re ${money(remaining)} from free standard shipping.` : 'A little extra good: free standard shipping unlocked.'}</span><div class="shipping-progress"><i style="width:${Math.min(order.subtotal / 4000 * 100, 100)}%"></i></div></div><dl class="totals"><div><dt>Subtotal</dt><dd>${money(order.subtotal)}</dd></div><div><dt>Shipping</dt><dd>${order.shipping ? money(order.shipping) : 'On us'}</dd></div><div><dt>Tax</dt><dd>Calculated at payment</dd></div><div class="grand-total"><dt>Total <small>before tax</small></dt><dd><small>USD</small> ${money(order.total)}</dd></div></dl>`;
  document.querySelectorAll('[data-shipping-price]').forEach(el => el.textContent = el.dataset.shippingPrice === 'standard' && !remaining ? 'FREE' : money(shippingOptions.find(s => s.id === el.dataset.shippingPrice).amount));
  items.querySelectorAll('[data-quantity]').forEach(button => button.onclick = () => { const next = getCart().map(p => p.id === button.dataset.quantity ? { ...p, quantity: p.quantity + Number(button.dataset.delta) } : p).filter(p => p.quantity > 0); try { saveCart(next); } catch { toast('Your bag could not be saved.'); } });
  items.querySelectorAll('[data-remove]').forEach(button => button.onclick = () => { try { saveCart(getCart().filter(p => p.id !== button.dataset.remove)); } catch { toast('Your bag could not be saved.'); } });
}
async function pay(event) {
  event.preventDefault();
  const button = document.querySelector('#pay-button');
  const error = document.querySelector('#checkout-error');
  button.disabled = true; error.textContent = ''; button.querySelector('span').textContent = 'Getting the good stuff ready…';
  try {
    const body = { name: document.querySelector('#name').value.trim(), email: document.querySelector('#email').value.trim(), items: getCart(), shipping };
    const fingerprint = JSON.stringify({ ...body, userId: user?.id });
    let attempt = storage.get('orlast-checkout');
    if (!attempt || attempt.fingerprint !== fingerprint) {
      const accessToken = [...crypto.getRandomValues(new Uint8Array(32))].map(n => n.toString(16).padStart(2, '0')).join('');
      attempt = { key: crypto.randomUUID(), accessToken, fingerprint, items: body.items };
      storage.set('orlast-checkout', attempt);
    }
    storage.set(`orlast-order-${attempt.key}`, { token: attempt.accessToken, items: attempt.items });
    const result = await api('/checkout', { method: 'POST', headers: { 'Idempotency-Key': attempt.key }, body: JSON.stringify({ ...body, accessToken: attempt.accessToken }) });
    if (result.status) { location.assign(`./order.html?order=${encodeURIComponent(result.orderId)}`); return; }
    const url = new URL(result.url);
    if (url.protocol !== 'https:' || url.hostname !== 'checkout.stripe.com') throw new Error('The payment link could not be opened. Please try again.');
    location.assign(url.href);
  } catch (err) { if (err.status === 409) sessionStorage.removeItem('orlast-checkout'); error.textContent = err.message; button.disabled = !getCart().length; button.querySelector('span').textContent = 'Continue to secure payment'; }
}
function auth() {
  const signup = page === 'signup';
  const next = new URLSearchParams(location.search).get('next') === 'checkout' ? 'checkout' : 'shop';
  const opposite = signup ? 'login' : 'signup';
  document.querySelector('#page-content').innerHTML = `<div class="auth-layout"><section class="auth-art"><div class="auth-photo"></div><div class="auth-photo-shade"></div><span class="auth-caption">YOUR EVERYDAY, A LITTLE BETTER.</span><div class="auth-round-stamp">${icon('cup')}<span>GOOD TO<br>HAVE YOU HERE.</span></div><div class="auth-editorial"><span class="overline">A LITTLE SPACE, JUST FOR YOU.</span><h2>${signup ? 'Good coffee.<br>Better company.' : 'Your kind<br>of familiar.'}</h2><p>${signup ? 'Come for the coffee.<br>Stay for the little everyday good.' : 'Same warm welcome.<br>A whole new good morning.'}</p><span class="auth-handwritten">${signup ? 'There’s a place for you here.' : 'Make yourself at home.'}</span></div><div class="auth-photo-footer"><span>ORLAST COFFEE</span><span>THOUGHTFULLY SOURCED. HAPPILY POURED.</span></div></section><section class="auth-form-wrap"><a class="back-link" href="./${next}.html">${icon('back')} ${next === 'checkout' ? 'Back to your bag' : 'Back to the good stuff'}</a><div class="auth-form-inner"><span class="overline">${signup ? 'BECOME A REGULAR' : 'HELLO AGAIN, COFFEE FRIEND'}</span><h1>${signup ? 'Your daily dose<br>of <em>belonging.</em>' : 'Welcome<br><em>back.</em>'}</h1><p class="auth-intro">${signup ? 'A familiar place for your next favorite.<br>Let’s make your everyday a little better.' : 'Good to see you again. Settle in,<br>your next favorite is waiting.'}</p><div id="signed-in-state"></div><form id="auth-form">${signup ? '<div class="field"><label for="name">Your name</label><input id="name" name="name" autocomplete="name" placeholder="What should we call you?" required minlength="2" maxlength="80"></div>' : ''}<div class="field"><label for="email">Email address</label><input id="email" name="email" type="email" autocomplete="email" placeholder="you@somewhere.nice" required maxlength="254"></div><div class="field"><label for="password">Password</label><div class="password-field"><input id="password" name="password" type="password" autocomplete="${signup ? 'new-password' : 'current-password'}" placeholder="${signup ? 'Make it a good one' : 'Your little secret'}" ${signup ? 'minlength="12"' : ''} maxlength="128" required><button type="button" id="toggle-password" aria-label="Show password" aria-pressed="false">${icon('eye')}</button></div>${signup ? '<small>At least 12 characters. Spaces and phrases welcome.</small>' : ''}</div><p class="form-error" id="auth-error" role="alert"></p><button class="primary-button" id="auth-submit"><span>${signup ? 'Make yourself at home' : 'Let’s get you settled'}</span>${icon('arrow')}</button><p class="auth-switch">${signup ? 'Already one of the regulars?' : 'New around here?'} <a href="./${opposite}.html?next=${next}">${signup ? 'Log in' : 'Find your place'}</a></p></form><div class="auth-guest"><span>${icon('flower')}</span><p>Just passing through? You’re welcome, too.<br><a href="./checkout.html">Continue as a guest ${icon('arrow')}</a></p></div><div class="auth-fine-print">${icon('lock')} Your details stay yours. Your coffee stays good.</div></div></section></div>`;
  document.querySelector('#toggle-password').onclick = event => { const input = document.querySelector('#password'); const show = input.type === 'password'; input.type = show ? 'text' : 'password'; event.currentTarget.setAttribute('aria-pressed', String(show)); event.currentTarget.setAttribute('aria-label', show ? 'Hide password' : 'Show password'); };
  document.querySelector('#auth-form').onsubmit = async event => {
    event.preventDefault();
    const button = document.querySelector('#auth-submit'); const error = document.querySelector('#auth-error');
    button.disabled = true; error.textContent = '';
    try {
      const body = Object.fromEntries(new FormData(event.currentTarget));
      await api(`/auth/${signup ? 'signup' : 'login'}`, { method: 'POST', body: JSON.stringify(body) });
...
async function orderPage() {
  ...
  try {
    if (!saved?.token) throw new Error('We can’t access this order from this tab. Please return to the browser tab you used to check out.');
    for (let attempt = 0; attempt < 8; attempt++) {
      const { order } = await api(`/orders/${encodeURIComponent(id)}`, { headers: { 'X-Order-Token': saved.token } });
      if (order.status === 'paid') {
        main.querySelector('h1').innerHTML = 'Good things.<br><em>Coming your way.</em>';
        document.querySelector('#order-status').textContent = 'Payment received. Thanks for making Orlast part of your everyday.';
        ...
        if (!saved.cleared) {
          const cart = getCart().map(p => ({ ...p, quantity: p.quantity - (saved.items.find(item => item.id === p.id)?.quantity || 0) })).filter(p => p.quantity > 0);
          saveCart(cart); storage.set(`orlast-order-${id}`, { ...saved, cleared: true }); sessionStorage.removeItem('orlast-checkout');
        }
        return;
      }
      if (['failed', 'expired'].includes(order.status)) { sessionStorage.removeItem('orlast-checkout'); throw new Error('This payment wasn’t completed. Your bag is saved. Return to checkout to try again.'); }
      if (attempt < 7) await new Promise(resolve => setTimeout(resolve, 2000));
    }
    document.querySelector('#order-status').textContent = 'Your payment is still being confirmed. You can refresh this page in a moment; please don’t pay again.';
  } catch (error) {
...
shell();
window.addEventListener('bagchange', () => page === 'checkout' ? renderBag() : updateBag());
window.addEventListener('storage', () => page === 'checkout' ? renderBag() : updateBag());
if (page === 'shop') shop();
...
try {
  ...
  if (user) {
    ...
    link.textContent = 'Log out'; link.href = '#';
    link.onclick = async event => { event.preventDefault(); try { await api('/auth/logout', { method: 'POST', body: '{}' }); location.reload(); } catch (error) { toast(error.message); } };
    if (page === 'checkout') {
      document.querySelector('#name').value ||= user.name;
      document.querySelector('#email').value ||= user.email;
      document.querySelector('#guest-note').innerHTML = `${icon('cup')}<div><strong>Good to see you, ${esc(user.name.split(' ')[0])}.</strong><p>You’re logged in. Let’s get something good on its way.</p></div>`;
      document.querySelector('#signup-invite').hidden = true;
    }
    if (page === 'login' || page === 'signup') {
      document.querySelector('#auth-form').hidden = true;
      document.querySelector('#signed-in-state').innerHTML = `<p class="notice">You’re already logged in as ${esc(user.name)}.</p><a class="primary-button" href="./checkout.html">Back to your bag ${icon('arrow')}</a>`;
    }
  }
} catch { /* Guest shopping remains available when account lookup is unavailable. */ }
